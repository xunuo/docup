...
function generateDoc(source, options) {
  var api = [],
      ...
      organized = {},
      url = options.url,
      type = options.type ? options.type : 'markdown'
      ;
      ...
      _.each(params, function(param, index) {
        entryMarkdown.push(
          //interpolate('${num}. `${name}` (${type}): ${desc}', {
            interpolate('- __${name}__ ( ${type} ): ${desc}', {
            'desc': escape(param[2]),
            'name': param[1],
            'num':  index + 1,
            'type': escape(param[0])
  ...
  // get content
  var docMarkdownContent = docMarkdown.join('\n');
  
    
  //// pretty @example's content
  //var exampleBlocks = docMarkdownContent.match(/```js[.\s\S]*?```/gm);
  //if(exampleBlocks){
  ...
  //                prettyExampleContent = examplesBlockItem.replace(replaceRegx,'\n')
  //                ;
  //            docMarkdownContent = docMarkdownContent.replace(examplesBlockItem,prettyExampleContent);
  //        }
  //    }
  //}
  ...
  if(type == 'html'){
      ...
      var htmlTpl = options.htmlTpl,
          htmlStyle = options.htmlStyle
      ;
      
      // Get TPL
      var htmlTpl = htmlTpl;
      if (!htmlTpl) {
          htmlStyle = htmlStyle ? htmlStyle : 'default';
      ...
      var Showdown = require('showdown'),
          ...
          htmlContent = converter.makeHtml(docMarkdownContent)
          // back to div
             .replace(/\<\!--\ \/div\ --\>/g,'</div>')
             .replace(/\<\!--\ div\ --\>/g,'<div>')
      ...
      // ejs render
      var ejs = require('ejs');
      docMarkdownContent = ejs.render(htmlTpl, {
          title : options.title,
          htmlContent : htmlContent
      });
  }else{
      
      docMarkdownContent = docMarkdownContent.replace(/\<\!--\ \/div\ --\>/g,'')
             .replace(/\<\!--\ div\ --\>/g,'')
             .replace(/\<\!--\ div class="toc-container"\ --\>/g,'')
             .replace(/\<\!--\ div class="doc-container"\ --\>/g,'')
      ;
